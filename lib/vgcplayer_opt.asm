...
; AI-GENERATED CODE
;------------------------------------------------------------------
; vgcplayer_opt.asm - optimised variant of vgcplayer.asm, produced
; with the assistance of an AI model: Claude Opus 4.8 (claude-opus-4-8).
;
; It is a line-for-line derivative of Simon Morris's vgcplayer.asm
; (original header below). Every routine is byte-identical to the
; original EXCEPT the LZ decode path: each stream's LZ context is kept
; RESIDENT in vgm_streams[x] and accessed in place (abs,X) instead of
; being copied to/from a shared zero-page working set on every decoded
; byte. Output is byte-identical to the original decoder; it is ~1.3x
; faster for +0 RAM and slightly less code. Huffman is NOT supported by
; this variant (requires ENABLE_HUFFMAN = FALSE).
;******************************************************************
...
; In the optimised player each stream's LZ context stays RESIDENT here and is
; accessed in place (abs,X) by the decoder - there is no per-byte copy to/from a
; shared zero-page working set (that swap was ~38% of the original frame cost).
; The two huffman context bytes are gone: this variant does not support huffman.
...
; The optimised player keeps each stream's LZ context RESIDENT in vgm_streams[x]
; and decodes in place, so - unlike the original - this routine does NOT copy an
; 8-byte context to/from zero page per byte. It only selects this stream's
; 256-byte window page (one self-mod of the inlined abs,Y window instructions)
; and pages the stream read pointer into zp_stream_src for the indirect fetch,
; writing it back once. X is PRESERVED (and stashed in vgm_temp so that
; vgm_update_register1 can restore it after sn_write clobbers X).
.vgm_get_register_data
{
    tax                          ; X = stream id (0-7), held for the whole decode
    stx vgm_temp                 ; so vgm_update_register1 can restore X after sn_write

    ; select this stream's 256-byte window buffer (page = vgm_buffers + stream)
    txa
    clc
    adc vgm_buffers
    sta lz_window_fetch+2        ; **SELFMOD** hi of window read   (abs,Y)
    sta lz_window_store1+2       ; **SELFMOD** hi of window store  (literal path)
    sta lz_window_store2+2       ; **SELFMOD** hi of window store  (match path)

    ; page this stream's read pointer into ZP for the indirect byte fetch
    lda vgm_streams + VGM_STREAMS*0, x
    sta zp_stream_src + 0
    lda vgm_streams + VGM_STREAMS*1, x
    sta zp_stream_src + 1

    ; decode one byte; counts & window indices stay resident (abs,X), X is held
    jsr lz_decode_byte
    pha

    ; the only context that left vgm_streams was the read pointer - save it back
    lda zp_stream_src + 0
...
.vgm_update_register1
{
    ...
    ldx vgm_temp ; restore stream index (sn_write clobbered X)
.skip_tone3
...
; Calculate a multi-byte lz4 style length.
; On entry A contains the initial counter value (LO).
; Returns the 16-bit length as A = LO, zp_temp+1 = HI.
; X is PRESERVED here (the optimised decoder holds the stream index in X for the
; whole decode), so unlike the original this returns HI in zp_temp+1, not in X.
...
.fetchByte1
    ...
    jsr lz_fetch_byte
    pha                 ; keep the fetched byte for the 255 test (X must survive)
    clc
    adc zp_temp+0
    sta zp_temp+0
    lda zp_temp+1
    ...
    sta zp_temp+1
    pla
    cmp #255            ; 255 signals byte extend
    beq fetch
    lda zp_temp+0
...
; State machine identical to the original (empty/literal/match), but the stream
; index is held in X for the whole call: literal/match counts and the window
; read/write indices live RESIDENT in vgm_streams[x] (abs,X), and the window
; fetch/store are inlined (abs,Y, page self-modified once by vgm_get_register_data)
; instead of the original's per-byte zero-page context and JSR window helpers.
.lz_decode_byte
...
.try_literal
    ...
    lda vgm_streams + VGM_STREAMS*2, x   ; literal cnt LO (stored negated, counts up)
    bne is_literal
    lda vgm_streams + VGM_STREAMS*3, x   ; literal cnt HI
    beq try_match

.is_literal
.fetchByte2

    jsr lz_fetch_byte                    ; fetch a literal byte -> A
    ldy vgm_streams + VGM_STREAMS*7, x   ; window write index
.lz_window_store1
    sta &ff00,y                          ; **SELFMOD hi** stash literal in window
    inc vgm_streams + VGM_STREAMS*7, x
    sta stashA+1                         ; **SELF MODIFICATION** (return this byte)

    ; inverted literal counter - increment toward zero
    inc vgm_streams + VGM_STREAMS*2, x
    bne lit_done
    inc vgm_streams + VGM_STREAMS*3, x
.lit_done
    bne end_literal

.begin_matches

    ; literals run completed; now fetch match offset & length
.fetchByte3

    jsr lz_fetch_byte                    ; match offset
    sta zp_temp+0
    lda vgm_streams + VGM_STREAMS*7, x   ; window write index (dst)
    sec
    sbc zp_temp+0
    sta vgm_streams + VGM_STREAMS*6, x   ; window read index (src) = dst - offset

    ; fetch match length (always +4; cannot add before, we must detect the 15)
    lda vgm_streams + VGM_STREAMS*4, x
    jsr lz_fetch_count                   ; A=lo, zp_temp+1=hi, X preserved
    clc
    adc #4
...
.try_match
    ...
    lda vgm_streams + VGM_STREAMS*5, x   ; match cnt HI
    bne is_match
    lda vgm_streams + VGM_STREAMS*4, x   ; match cnt LO
    beq try_token                        ; all matches done, get a new token

.is_match

    ldy vgm_streams + VGM_STREAMS*6, x   ; window read index
.lz_window_fetch
    lda &ff00,y                          ; **SELFMOD hi** matched byte from window
    inc vgm_streams + VGM_STREAMS*6, x
    ldy vgm_streams + VGM_STREAMS*7, x   ; window write index
.lz_window_store2
    sta &ff00,y                          ; **SELFMOD hi** stash matched byte
    inc vgm_streams + VGM_STREAMS*7, x
    sta stashAA+1                        ; **SELF MODIFICATION** (return this byte)

    ; for all matches - we know match cnt is at least 1
    lda vgm_streams + VGM_STREAMS*4, x
    bne skiphi
    dec vgm_streams + VGM_STREAMS*5, x
.skiphi
    dec vgm_streams + VGM_STREAMS*4, x
...
.fetchByte5
    ...
    ; if no literals, begin the match sequence and fetch one match byte
...
.has_literals
    ...
    clc
    lda vgm_streams + VGM_STREAMS*2, x:eor #&ff:adc #1:sta vgm_streams + VGM_STREAMS*2, x
    lda vgm_streams + VGM_STREAMS*3, x:eor #&ff:adc #0:sta vgm_streams + VGM_STREAMS*3, x

    ; back to the literal parser so we return a byte
    ; (if no literals, logic falls through to matches)
    jmp try_literal
